...
if (!connectionString) {
  console.warn(
    "[⚠️] POSTGRES_URL/DATABASE_URL is not set. Form submissions will fail."
  );
}
...
async function sendConfirmationEmail(payload: { name: string; email: string }) {
  if (!resend) {
    throw new Error(
      "Resend API key ausente. O lead não receberá confirmação por e-mail."
    );
  }
  ...
  if (error) {
    throw new Error(
      `[❌] Erro ao enviar confirmação ao lead: ${error.message ?? error}`
    );
  }
}
...
export async function POST(req: NextRequest) {
  if (!pool) {
    return NextResponse.json(
      { status: "error", message: "Configuração do banco de dados ausente." },
      { status: 500 }
    );
  }
